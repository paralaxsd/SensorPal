using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace SensorPal.Server.Migrations
{
    /// <inheritdoc />
    public partial class Initial : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "MonitoringSessions",
                columns: table => new
                {
                    Id = table.Column<long>(type: "INTEGER", nullable: false)
                        .Annotation("Sqlite:Autoincrement", true),
                    StartedAt = table.Column<DateTimeOffset>(type: "TEXT", nullable: false),
                    EndedAt = table.Column<DateTimeOffset>(type: "TEXT", nullable: true),
                    EventCount = table.Column<int>(type: "INTEGER", nullable: false),
                    BackgroundFile = table.Column<string>(type: "TEXT", nullable: true)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_MonitoringSessions", x => x.Id);
                });

            migrationBuilder.CreateTable(
                name: "NoiseEvents",
                columns: table => new
                {
                    Id = table.Column<long>(type: "INTEGER", nullable: false)
                        .Annotation("Sqlite:Autoincrement", true),
                    SessionId = table.Column<long>(type: "INTEGER", nullable: false),
                    DetectedAt = table.Column<DateTimeOffset>(type: "TEXT", nullable: false),
                    PeakDb = table.Column<double>(type: "REAL", nullable: false),
                    DurationMs = table.Column<int>(type: "INTEGER", nullable: false),
                    BackgroundOffsetMs = table.Column<long>(type: "INTEGER", nullable: false),
                    ClipFile = table.Column<string>(type: "TEXT", nullable: true)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_NoiseEvents", x => x.Id);
                    table.ForeignKey(
                        name: "FK_NoiseEvents_MonitoringSessions_SessionId",
                        column: x => x.SessionId,
                        principalTable: "MonitoringSessions",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_NoiseEvents_SessionId",
                table: "NoiseEvents",
                column: "SessionId");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "NoiseEvents");

            migrationBuilder.DropTable(
                name: "MonitoringSessions");
        }
    }
}
